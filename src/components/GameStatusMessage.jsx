...
export function GameStatusMessage({ message, maxNumber, proximity, feedbackToken = 0 }) {
  ...
  if (proximity && typeof proximity === 'object') {
    ...
    return <>
      <div className={`direction-feedback proximity-feedback proximity-${proximity.level} ${high ? 'too-high' : 'too-low'}`} key={`${proximity.level}-${proximity.direction}-${feedbackToken}`} role="status" aria-live="polite" aria-label={accessibleLabel}>
        <AppImage src={STATE_IMAGES[almost ? 'almost' : high ? 'high' : 'low']} alt="" decorative className="feedback-state-image" />
  ...
  if (correct) {
    return <><div className="correct-feedback" role="status" aria-live="polite"><AppImage src={STATE_IMAGES.win} alt="Celebration after a correct guess" className="feedback-state-image" /><span aria-hidden="true">🎯</span><strong>CORRECT!</strong><span className="correct-feedback-detail">Correct Number</span></div><p className="rang">Guess a Number between 1 and {maxNumber}</p></>;
  }
  if (tooHigh || tooLow) {
    const high = tooHigh;
    return <>
      <div className={`direction-feedback ${high ? 'too-high' : 'too-low'}`} key={`${message}-${feedbackToken}`} role="status" aria-live="polite">
        <AppImage src={STATE_IMAGES[high ? 'high' : 'low']} alt="" decorative className="feedback-state-image" />
