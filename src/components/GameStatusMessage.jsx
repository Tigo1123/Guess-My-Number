  ...
  if (proximity && typeof proximity === 'object') {
    ...
    return <>
      <div className={`direction-feedback proximity-feedback proximity-${proximity.level} ${high ? 'too-high' : 'too-low'}`} key={`${proximity.level}-${proximity.direction}-${feedbackToken}`} role="status" aria-live="polite" aria-label={accessibleLabel}>
        <div className="feedback-artwork" aria-hidden="true"><AppImage src={STATE_IMAGES[almost ? 'almost' : high ? 'high' : 'low']} alt="" decorative className="feedback-state-image" loading="eager" /></div>
        <span className="direction-feedback-copy"><strong><span aria-hidden="true">{almost ? '🔥' : high ? '⬆️' : '⬇️'}</span> {levelLabel}</strong><span>{guidance} <b aria-hidden="true">{high ? '↑' : '↓'}</b></span></span>
      </div>
  ...
  if (tooHigh || tooLow) {
    ...
    return <>
      <div className={`direction-feedback ${high ? 'too-high' : 'too-low'}`} key={`${message}-${feedbackToken}`} role="status" aria-live="polite">
        <div className="feedback-artwork" aria-hidden="true"><AppImage src={STATE_IMAGES[high ? 'high' : 'low']} alt="" decorative className="feedback-state-image" loading="eager" /></div>
        <span className="direction-feedback-copy"><strong><span aria-hidden="true">{high ? '⬆️' : '⬇️'}</span> {high ? 'TOO HIGH' : 'TOO LOW'}</strong><span>{high ? 'Try a lower number' : 'Try a higher number'}</span></span>
      </div>
      <p className="rang">Guess a Number between 1 and {maxNumber}</p>
    </>;
  }
